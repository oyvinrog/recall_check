...
async fn evaluate_recall(payload: EvaluationRequest) -> Result<EvaluationResponse, String> {
  let entry = Entry::new(KEYRING_SERVICE, KEYRING_USER)
    .map_err(|err| err.to_string())?;
  let api_key = entry.get_password().map_err(|_| "Missing API key".to_string())?;
...
fn main() {
  tauri::Builder::default()
    .invoke_handler(tauri::generate_handler![get_api_key_status, set_api_key, evaluate_recall])
